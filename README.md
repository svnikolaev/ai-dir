...
Теперь для каждого файла выводится общее количество строк, а если в файле есть функции или методы длиннее 20 строк, они отображаются рядом с именем файла с указанием количества строк (например, `(long: my_function:25, another:42)`). Это помогает быстро оценить сложность файла и найти кандидатов на рефакторинг.
